...
def build_mock_pdf(attempt, username: str, next_level_unlocked: bool):
    ...
    regular_font_path = os.path.join(font_path, "Lexend-Regular.ttf")
    print("📁 FONT PATH:", regular_font_path)
    try:
        pdfmetrics.registerFont(TTFont("Lexend", regular_font_path))
        FONT = "Lexend"
    ...
    # 🎨 STYLES
    title = ParagraphStyle("title", fontName=FONT, fontSize=18, textColor=PRIMARY, spaceAfter=6)
    section = ParagraphStyle("section", fontName=FONT, fontSize=13, textColor=TEXT, spaceAfter=8)
    normal = ParagraphStyle("normal", fontName=FONT, fontSize=11, textColor=TEXT, leading=14)
    small = ParagraphStyle("small", fontName=FONT, fontSize=10, textColor=SUB, leading=13)
    ...
    # ---------------- SCORE ----------------
    words = attempt.results.get("words", []) if attempt.results else []
    if words:
        _word_scores = [float(w.get("score", 0) or 0) for w in words]
        score = round(sum(_word_scores) / len(_word_scores), 2)
    else:
    ...
    score_box = Table([
        ...
        [Paragraph(f"<b>Verdict:</b> {(attempt.verdict or 'keep_practicing').replace('_',' ').title()}", normal)],
    ...
    for w in words:
        ...
        word_block.append(
            Paragraph(f"<b>{w['expected']}</b> → {w.get('recognized', w.get('spoken', '-'))}", normal)
        )
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle("footer", fontName=FONT, fontSize=9, textColor=SUB)
    ))
